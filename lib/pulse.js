...
export class PulseSynthesizer {
  ...
  async synthesize(metrics = {}, targetDir = process.cwd()) {
    ...
    // What the source says it did NOT look at. A verdict can be honest and still be
    // narrower than the component it is scoring.
    let lookoutUnknowns = metrics.lookoutUnknowns ?? null;
    if (!lookoutVerdict) {
      try {
        const { LookoutAuditor } = await import('../../lookout/lib/lookout.js');
        const audit = new LookoutAuditor().auditPackageJson(path.join(targetDir, 'package.json'));
        lookoutVerdict = audit.verdict;
        lookoutUnknowns = audit.unknowns ?? null;
      } catch {
    ...
    // A PASS from a check that declares its own blind spots is not a clean dependency
    // tree; it is a clean manifest. lookout consults no advisory database and reads no
    // lockfile, so scoring its PASS as a full 10 for "dependency security" substitutes a
    // good default for evidence that was never gathered — the exact failure this module
    // was rewritten to stop, arriving one level further out than the last time.
    //
    // The component is still scored, because something real was measured. It is capped,
    // and what was not measured is carried into `unknowns` where a reader will meet it.
    const lookoutPartial = Array.isArray(lookoutUnknowns) && lookoutUnknowns.length > 0;
    if (lookoutVerdict && lookoutPartial) {
      for (const u of lookoutUnknowns) unknowns.push(`dependencySecurity: ${u}`);
    }
    ...
    const components = {
      ...
      dependencySecurity: verdictToScore(lookoutVerdict, lookoutPartial
        ? { PASS: 7, WARN: 5, DEFAULT: 2 }
        : { PASS: 10, WARN: 6, DEFAULT: 2 }),
    ...
    // A weighted mean lets three healthy components average away a catastrophic
    // one: PASS + PASS + README + a mutation score of 0 arrives at exactly 8.0,
    // which is "EXCELLENT" for a suite that kills no mutations at all. A summary
    // that can hide the worst dimension is not a summary of the project.
    //
    // So the floor overrides the mean: any measured component at or below 2/10
    // caps the verdict, and the offending components are named in `critical`.
    const CRITICAL_FLOOR = 2;
    const critical = measured.filter(([, v]) => v <= CRITICAL_FLOOR).map(([k]) => k);

    // Partial coverage caps it too: "EXCELLENT" on one measured component out of
    // four is a statement about the sample, not about the project.
    let status;
    if (critical.length) status = 'CRITICAL';
    else if (healthScore >= 8.0) status = coverage >= 0.75 ? 'EXCELLENT' : 'GOOD_PARTIAL';
    ...
    return {
      ...
      honest:
        ...
        (critical.length
          ? `Status forced to CRITICAL: ${critical.join(', ')} at or below ${CRITICAL_FLOOR}/10 — a mean would have hidden this.`
          : ''),
    };
  }
}
